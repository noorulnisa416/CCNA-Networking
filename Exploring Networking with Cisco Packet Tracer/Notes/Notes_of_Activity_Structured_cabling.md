...
9. [Important Tips & Warnings](#9-important-tips--warnings)
10. [Quick Revision Glossary](#10-quick-revision-glossary)
...
**Definition:** A method of **permanently terminating a wire** into a connector block. The wire is "punched" into a slot that pierces through the insulation to make an electrical contact — no stripping needed.
...
**Definition:** A **network outlet installed on a wall** that provides RJ45 ports for connecting end devices (PCs, printers) to the network. It connects to the patch panel through cabling routed inside the walls.
...
### 🔹 Cable Pegboard
**Definition:** In Cisco Packet Tracer's Physical mode, the **Cable Pegboard** is the tool panel where you select which type of cable to use for connections — like picking a tool from a toolbox.
...
**Definition:** A **1 Gbps (Gigabit per second) network port** on a switch. Each port is identified with a specific number.
...
### 🔹 BendPoint
**Definition:** A feature in Packet Tracer Physical mode that lets you **redirect a cable's path** by creating a bend/anchor point and dragging it along the wall or floor — simulating how real cables are routed inside walls.
...
**Definition:** A service/protocol that **automatically assigns IP addresses** and other network settings to devices when they connect to the network.
...
| **e** | *(Optional)* Right-click white space in rack → **Manage All Cables on Rack** → auto-organizes all cables |
...
> 💡 **Pro Tip:** Right-click the switch or patch panel → **Inspect Front** to zoom in and see the small port numbers more clearly. You can also use the **Zoom In** tool on the toolbar.
...
| Step | What to Do |
|---|---|
| **a** | Click **Connections** in the **Device-Type Selection Box** → click **Structured Cabling** |
...
| **e** | Click **Wall Mount0** → select **PunchDown1** → click **Equipment Cabinet** → select **Rack → Patch Panel0 → Punchdown13** |
...
> ⏳ **Wait 1–2 minutes** after connecting Office-Admin and Printer0 for DHCP to assign IP addresses from the Office-Server.
...
> 💡 **Remember:** Using BendPoints in Packet Tracer **simulates running cables through walls** — the same effect as real in-wall cable routing!
...
> Creating BendPoints and dragging cables along walls in Packet Tracer simulates how cables are run inside walls, ceilings, and floors in a real office environment.

---

## 10. Quick Revision Glossary
